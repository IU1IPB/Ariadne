...
The realTimeAprsGet.py program it’s a never-ending daemon that check continuously the APRS network in order to answer to requests to ANSRVR group : it’s execution it’s not mandatory (loosing the ANSRVR capability), but it should be started as SYSTEMCTL service (or equivalent in Windows) for allowing automatic restart after error. For Linux system, the systemctl service definition is provided.
